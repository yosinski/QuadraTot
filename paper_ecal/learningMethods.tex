We implemented two classes of algorithms to produce
...
appropriate fixed constants ($C_O = 40$ for outer
...
position for each motor (numbered in \figref{topdown2.jpg}), as a vector function of time, are produced by the following nine functions (listed in the order):
...
\emph{Uniform random hill climbing}: This method always starts
  with the current best gait, and then chooses the next $\vec{\theta}$ by
  ...
  the new best gait. The process is then repeated.
