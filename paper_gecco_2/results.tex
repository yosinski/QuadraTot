...
\tabref{results}.  The best overall gait for
...
% No single strategy
%consistently beat the others.
%: for the first trial Linear Regression
...
%\figref{hnResults} and \tabref{results}.  
...
%66\% better than the best
%non-HyperNEAT gait and 
...
(\figref{hnResults}). For example, we tested an example HyperNEAT
...
%We believe the slope of the HyperNEAT learning
%curve would be steeper if the noise in the evaluations could be
%reduced.

\acmFig{hnResults}{.86}{Average fitness ($\pm $ SE) of the highest
  ...
  were halved if the gait overly stressed the motors,
  so the true performance without this penalty would
  be much higher.}
