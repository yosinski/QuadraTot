...
Much research has been done in this subject, and previous work has shown that learned gaits can outperform designed gaits \cite{valsalam:mii}, \cite{kohl:stone}. %JMC - I have a similar claim in my CEC 2009 paper. Grab those cites for here (especially the Hornby ones). 
...
\cite{valsalam:mii,clune2,clune1,clune3}. %JMC-note how you can put multiple cites in one bracket...but no spaces after the comma!
However, in most previous works, the experimenter has had to manually decide the regularities of the gaits. %we need cites for this claim. you can get them from Clune1
...
A follow-up paper by Glette et al. evolved gaits with HyperNEAT directly in hardware on the QuadraTot robot platform and found that HyperNEAT's gaits outperformed manually designed parameterized learning algorithms, but still did not produce impressive, natural gaits \cite{yos:clune}. % insert (figure 1 -- pic of quadratot) 
A further follow-up study built a simulator for the QuadraTot robot platform to test whether the inclusion of a simulator would improve results and found that it did:  an evolutionary algorithm with direct encoding in simulation produced gaits that outperformed those gaits evolved by HyperNEAT in hardware~\cite{glette}. %JMC-Should there be a space between the end of the sentence and the cite? Whatever the answer, we should be consistent. If so, I have added here with the ~. The ~ tells Latex not to split that space from the character before and after it. 
