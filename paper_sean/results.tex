...
fast, organic gaits. In simulation, HyperNEAT gaits were significantly faster than those from Glette et al. (\figref{threeplots}-Top, $\emph{p} < 6.8\times10^{-8}$, comparing the best gaits in the final generation of each run via MATLAB's Wilcoxon rank sum test).  
Specifically, HyperNEAT gaits were 55\% faster in simulation (26.9
cm/s, 95\% CIs 23.8 cm/s and 26.750 cm/s versus 16.7 cm/s $\pm$ 1.9 SD, \tabref{results}).
...
{
\textbf{Top: }
HyperNEAT outperforms a genetic algorithm with a simple encoding when both algorithms are combined with a simulator. Plotted are medians over 20 runs in simulation (solid lines) $\pm$ CIs (dashed lines). HyperNEAT gaits are 61.1\% faster in
...
However, we can likely rule out the noise hypothesis because we performed 20 
runs in simulation with the same evaluation samples as 
in Yosinski et al.~\cite{yos:clune} and saw that the results were not 
significantly different (mean fitness of 7.9 cm/s), making the likely explanation 
the number of evaluations performed.
Another factor for high-perfoming gaits may be due to
