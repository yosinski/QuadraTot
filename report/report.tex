...
quickly, efficiently, or with as little power as possible.
Oftentimes, these optimizations are done manually by an expert who
designs a gait to be, for example, as fast as possible \cite{someone}.  Some groups (Kohl and Stone) have 

Previous
Walking robots often have


Many previous walking robots have had walking motions -- or gaits --
designed and tweaked by hand \cite{someone,else}, or in some cases, beginning with a 

We are using machine learning to design a gait for a quadruped
robot. The code so far consists of the robot class, optimization
class, parameterized model, sine model, and camera feedback.
